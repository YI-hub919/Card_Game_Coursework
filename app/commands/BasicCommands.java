...
public class BasicCommands {
	...
	/**
	 * BasicCommands class integrating card information display and zoom functionality.
	 * Fully compliant with Class List and Story Card design requirements:
	 * - Card contains MiniCard and BigCard
	 * - CreatureCard and SpellCard inherit from Card
	 * - Supports enlarged BigCard display when selected
	 */
	public class BasicCommands {
		...
		public static class MiniCard {
			...
			public MiniCard(String imageName) {
			...
			public void nextFrame() {
				animationFrame = (animationFrame + 1) % 10; // Simulate animation loop
			}

			public String getDisplayInfo() {
				return String.format("[MiniCard] %s | Frame: %d", imageName, animationFrame);
			}
		}

		// ======================== BigCard Class ========================
		public static class BigCard {
			private String enlargedImage;
			private String description;

			public BigCard(String enlargedImage, String description) {
			...
			public String getEnlargedDisplay() {
		...
		public abstract static class Card {
			...
			public Card(String name, int manaCost, String miniImage, String bigImage, String bigDesc) {
				this.name = name;
				this.manaCost = manaCost;
				this.miniCard = new MiniCard(miniImage);
				this.bigCard = new BigCard(bigImage, bigDesc);
			}
		...
		// ======================== Creature Card Class ========================
		public static class CreatureCard extends Card {
			private int attack;
			private int health;

			public CreatureCard(String name, int manaCost, int attack, int health, 
							String miniImage, String bigImage) {
				super(name, manaCost, miniImage, bigImage,
						String.format("Creature: %s | Mana Cost: %d | Attack: %d | Health: %d", 
			...
			public String getCardInfo() {
				return String.format("Creature: %s | Mana Cost: %d | Attack: %d | Health: %d",
		...
		public static class SpellCard extends Card {
			...
			public SpellCard(String name, int manaCost, String effect, 
							String miniImage, String bigImage) {
				super(name, manaCost, miniImage, bigImage,
						String.format("Spell: %s | Mana Cost: %d | Effect: %s", 
									name, manaCost, effect));
				this.effect = effect;
			}

			@Override
			public String getCardInfo() {
				return String.format("Spell: %s | Mana Cost: %d | Effect: %s",
		...
		// ======================== Core Card Display Methods ========================
		/**
		 * Display enlarged card information when selected
		 * @param card The card to display (creature/spell)
		 */
		public void showEnlargedCardInfo(Card card) {
			if (card == null) {
				System.out.println("Invalid card, no information to display");
				return;
			}
			// Use BigCard for enlarged display
			System.out.println(card.getBigCard().getEnlargedDisplay());
		}

		/**
		 * Animate the mini card display
		 * @param card The card to animate
		 */
		public void animateMiniCard(Card card) {
			if (card != null) {
				card.getMiniCard().nextFrame();
				System.out.println(card.getMiniCard().getDisplayInfo());
			}
		}

		// ======================== Main Method (Functional Verification) ========================
		public static void main(String[] args) {
			BasicCommands commands = new BasicCommands();

			// Test 1: Creature Card with enlarged display
			Card wolf = new CreatureCard("Werewolf", 2, 2, 2, 
										"wolf_mini.png", "wolf_big.png");
			System.out.println("=== Testing Creature Card ===");
			commands.showEnlargedCardInfo(wolf);
			commands.animateMiniCard(wolf);

			// Test 2: Spell Card with enlarged display
			Card fireball = new SpellCard("Fireball", 3, "Deal 4 damage to a single target",
										"fireball_mini.png", "fireball_big.png");
			System.out.println("\n=== Testing Spell Card ===");
			commands.showEnlargedCardInfo(fireball);
			commands.animateMiniCard(fireball);

			// Test 3: Null card boundary scenario
			System.out.println("\n=== Testing Null Card ===");
			commands.showEnlargedCardInfo(null);

			// Additional demonstration: Dragon creature + Heal spell
			Card dragon = new CreatureCard("Dragon", 8, 8, 8,
										"dragon_mini.png", "dragon_big.png");
			Card heal = new SpellCard("Healing Touch", 1, "Restore 5 health",
									"heal_mini.png", "heal_big.png");
			
			System.out.println("\n=== Additional Demonstrations ===");
			commands.showEnlargedCardInfo(dragon);
			commands.animateMiniCard(dragon);
			commands.showEnlargedCardInfo(heal);
			commands.animateMiniCard(heal);
		}
	}
}
